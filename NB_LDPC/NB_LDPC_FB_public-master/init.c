...
void LoadCode(char *FileMatrix, code_t *code) {
    ...
    // if ( code->columnDegree == NULL ) err(EXIT_FAILURE,"%s:%d > malloc failed
    // !",__FILE__,__LINE__);
    for (n = 0; n < N; n++)
    ...
    for (m = 0; m < M; m++) {
        ...
        // if ( code->mat[m] == NULL ) err(EXIT_FAILURE,"%s:%d > malloc failed
        // !",__FILE__,__LINE__);
    }
    code->matValue = calloc(M, sizeof(int *));
    for (m = 0; m < M; m++) {
        code->matValue[m] = calloc(code->rowDegree[m], sizeof(int));
        // if ( code->matValue [m] == NULL ) err(EXIT_FAILURE,"%s:%d > malloc failed
        // !",__FILE__,__LINE__);
    }
...
void LoadTables(table_t *table, int GF, int logGF) {
    ...
    if (GF != 16 && GF != 32 && GF != 64 && GF != 256 && GF != 4096) {
        printf("The binary image of GF(%d) is not available in this version of the program. Please "
               "try GF(64) or GF(256)\n",
               GF);
        exit(EXIT_FAILURE);
    }
