...
/*!
 * \fn void ModelChannel_AWGN_BPSK (code_t *code, decoder_t *decoder, table_t *table, int **NBIN,
 * float EbN,int *init_rand)
...
void ModelChannel_AWGN_BPSK_repeat(code_t *code, decoder_t *decoder, table_t *table, int **NBIN,
                                   float EbN, int *init_rand) {
    const int N = code->N;
    const int logGF = code->logGF;
    int n, k, g, q;
    ...
    for (n = 0; n < N; n++) {
        ...
        for (r = 0; r < repeat; r++) {
            ...
            for (g = 0; g < code->GF; g++) {
                ...
                for (q = 0; q < logGF; q++) {
                    // TMP[g] = TMP[g] +
                    // SQR(NoisyBin[n][q]-BPSK(table->BINGF[g][q]))/(2.0*SQR(sigma));
                    TMP[g] = TMP[g] - NoisyBin[n][q] * BPSK(table->BINGF[g][q]);
                }
            }
        }
...
/*!
 * \fn ModelChannel_AWGN_64 (code_t *code, decoder_t *decoder, table_t *table, int **NBIN, float
 * EbN, int *init_rand)
...
void ModelChannel_AWGN_64(code_t *code, decoder_t *decoder, int **NBIN, float EbN, int *init_rand) {
    ...
    ////compute normalization factor so that average power of one point of constellation is equal to
    ///one
    float norm_factor = 0.0;
    for (i = 0; i < code->GF; i++) {
        // norm_factor = table_64QAM[i][0]*table_64QAM[i][0] +
        // table_64QAM[i][1]*table_64QAM[i][1]+norm_factor;//compute sum
        norm_factor = table_64APSK[i][0] * table_64APSK[i][0] +
                      table_64APSK[i][1] * table_64APSK[i][1] + norm_factor; // compute sum
    }
    ...
    for (n = 0; n < N; n++) {
        ...
        for (q = 0; q < 2; q++) {
            ...
            /* BPSK modulation + AWGN noise (Box Muller method for Gaussian sampling) */
#ifdef rayleigh_fading
            NoisyBin[n][q] = modulation[som][q] * attenuation_temp +
                             sigma * sqrt(-2.0 * log(u)) * cos(2.0 * PI * v);
#else
            NoisyBin[n][q] = modulation[som][q] + sigma * sqrt(-2.0 * log(u)) * cos(2.0 * PI * v);
#endif
        }
        // printf("%f %f %f %f \n",NoisyBin[n][0], modulation[som][0] , NoisyBin[n][1] ,
        // modulation[som][1]);getchar();
    }
    ...
    for (n = 0; n < N; n++) {
        ...
        for (k = 0; k < code->GF; k++) {
            ...
            if (attenuation[n] > 0)
                TMP[k] =
                    SQR(NoisyBin[n][0] - modulation[som][0] * attenuation[n]) / (2.0 * SQR(sigma)) +
                    SQR(NoisyBin[n][1] - attenuation[n] * modulation[som][1]) / (2.0 * SQR(sigma));
            else
                TMP[k] = 0;
#else
...
/*!
 * \fn ModelChannel_AWGN_256QAM(code_t *code, decoder_t *decoder, table_t *table, int **NBIN, float
 * EbN, int *init_rand)
...
void ModelChannel(code_t *code, decoder_t *decoder, int **NBIN, float EbN, int *init_rand) {
    ...
    if (GF == 64) {
        ...
        float table_APSK[64][2] = {{2.2 * cos(PI * 25 / 16), 2.2 * sin(PI * 25 / 16)},
    ...
    float norm_factor = 0.0;
    for (i = 0; i < GF; i++) {
        norm_factor = table_GF[i][0] * table_GF[i][0] + table_GF[i][1] * table_GF[i][1] +
                      norm_factor; // compute sum
    }
    norm_factor = sqrt(code->GF / norm_factor);
    // printf(" norm_factor = %f ", norm_factor); getchar();
    ...
    for (n = 0; n < N; n++) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            NoisyBin[n][i] = table_GF[som][i] * attenuation_temp +
                             sigma * sqrt(-2.0 * log(u)) * cos(2.0 * PI * v);
        }
        // printf("%f %f %f %f \n",NoisyBin[n][0], table_GF[som][0] , NoisyBin[n][1] ,
        // table_GF[som][1]);getchar();
    }
    ...
    for (n = 0; n < N; n++) {
        ...
        for (k = 0; k < GF; k++) {
            ...
            if (attenuation[n] > 0)
                TMP[k] =
                    SQR(NoisyBin[n][0] - table_GF[som][0] * attenuation[n]) / (2.0 * SQR(sigma)) +
                    SQR(NoisyBin[n][1] - attenuation[n] * table_GF[som][1]) / (2.0 * SQR(sigma));
            else
                TMP[k] = 0;
#else
#ifdef rayleigh_fading_SSD
            ...
            if (attenuation[n][0] > 0) {
                TMP[k] =
                    SQR(NoisyBin[n][0] - attenuation[n][0] * table_GF[som][0]) / (2.0 * SQR(sigma));
            }

            if (attenuation[n][1] > 0) {
                TMP[k] = TMP[k] + SQR(NoisyBin[n][1] - attenuation[n][1] * table_GF[som][1]) /
                                      (2.0 * SQR(sigma));
            }

#else
            TMP[k] = +SQR(NoisyBin[n][0] - table_GF[som][0]) / (2.0 * SQR(sigma)) +
                     SQR(NoisyBin[n][1] - table_GF[som][1]) / (2.0 * SQR(sigma));
#endif
#endif // rayleigh_fading
...
void ModelChannel_AWGN_256QAM_4D(code_t *code, decoder_t *decoder, int **NBIN, float EbN,
                                 int *init_rand) {
    const int N = code->N;
    int n, k, g, q;
    float u, v, sigma;
    ...
    float **NoisyBin = calloc(N, sizeof(float *));
    for (n = 0; n < N; n++)
        NoisyBin[n] = calloc(4, sizeof(float));
    /* Binary-input AWGN channel : */
    int i;
    ...
    for (i = 0; i < code->GF; i++) {
        norm_factor = table_256[i][0] * table_256[i][0] + table_256[i][1] * table_256[i][1] +
                      norm_factor; // compute sum
        norm_factor = table_256[i][2] * table_256[i][2] + table_256[i][3] * table_256[i][3] +
                      norm_factor; // compute sum
    }
    ...
    for (n = 0; n < N; n++) {
        ...
        for (q = 0; q < 4; q++) {
            ...
            NoisyBin[n][q] = table_256[som][q] * attenuation_temp +
                             sigma * sqrt(-2.0 * log(u)) * cos(2.0 * PI * v);
        ...
        // printf("%f %f %f %f \n",NoisyBin[n][0], table_256[som][0] , NoisyBin[n][1] ,
        // table_256[som][1]);getchar();
    }
    ...
    for (n = 0; n < N; n++) {
        ...
        for (k = 0; k < code->GF; k++) {
            ...
            TMP[k] = 0;
            if (attenuation[n][0] > 0) {
                TMP[k] = SQR(NoisyBin[n][0] - attenuation[n][0] * table_256[som][0]) /
                         (2.0 * SQR(sigma));
            }

            if (attenuation[n][1] > 0) {
                TMP[k] = TMP[k] + SQR(NoisyBin[n][1] - attenuation[n][1] * table_256[som][1]) /
                                      (2.0 * SQR(sigma));
            }

            if (attenuation[n][2] > 0) {
                TMP[k] = TMP[k] + SQR(NoisyBin[n][2] - attenuation[n][2] * table_256[som][2]) /
                                      (2.0 * SQR(sigma));
            }

            if (attenuation[n][3] > 0) {
                TMP[k] = TMP[k] + SQR(NoisyBin[n][3] - attenuation[n][3] * table_256[som][3]) /
                                      (2.0 * SQR(sigma));
            }
...
// [新增] BSC 信道模型
// p_error: 翻转概率 (对应命令行输入的 EbN 参数，这里我们把它当作概率 p 使用)
void ModelChannel_BSC(code_t *code, decoder_t *decoder, table_t *table, int **NBIN, float p_error,
    ...
    // BSC 信道的对数似然比 (LLR) 常数
    // LLR = log((1-p)/p)
    // 如果 p_error 非常小，LLR 会很大
    if (p_error < 1e-10)
        p_error = 1e-10; // 防止除零
    float LLR_val = log((1.0 - p_error) / p_error);

    float TMP[4096]; // 临时存储

    for (n = 0; n < N; n++) {
        // 1. 计算该符号下，所有 GF 元素的初始 LLR
        for (g = 0; g < code->GF; g++) {
            TMP[g] = 0.0;
            ...
            for (q = 0; q < logGF; q++) {
                // 获取发送的真实比特 (来自 NBIN)
                int sent_bit = NBIN[n][q];

                // 模拟 BSC 翻转：生成随机数决定是否翻转
                // 注意：这里其实不需要显式存储“接收比特”，我们可以直接计算 LLR
                // 如果 rand < p，则发生翻转 (Received != Sent) -> 贡献 -LLR
                // 如果 rand >= p，则未翻转 (Received == Sent) -> 贡献 +LLR
                // 但为了解码器，我们需要计算 P(Received | Candidate_Symbol_Bit)

                // 让我们模拟接收过程：
                int received_bit = sent_bit;
                rand_val = My_drand48(init_rand);
                if (rand_val < p_error) {
                    received_bit = 1 - sent_bit; // 翻转
                }

                // 计算候选符号 g 的第 q 位 (table->BINGF[g][q]) 与 接收比特 的匹配度
                int candidate_bit = table->BINGF[g][q];

                if (candidate_bit == received_bit) {
                    // 匹配：意味着该位可能是正确的，增加可信度
                    // LLR = log(P(r|0)/P(r|1)) if candidate is 0...
                    // 简而言之：硬判决下的软信息
                    // 累加 0 (最可信) 或者 这里的逻辑是 EMS 算法通常处理的是相对度量
                    // 我们这里直接累加汉明距离对应的 LLR 惩罚可能更直观
                    // 但为了适配 decode_soft 的 LLR 定义 (通常是 log(Prob))：
                    // TMP[g] += 0; // 或者是 log(1-p)
                } else {
                    // 不匹配：受到惩罚
                    TMP[g] -= LLR_val; // 减去 log((1-p)/p)
                }
            }
        }
