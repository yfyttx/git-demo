...
std::pair<std::vector<int>, std::vector<std::pair<int, int>>> getNandE(
    ...
    for (int n = 0; n < Hd.cols(); ++n) {
        if (Hd(m_prime, n) == 1) N.push_back(n);
    }
    std::vector<std::pair<int, int>> E;
    for (int n : N) {
        for (int m = 0; m < Hc.rows(); ++m) {
            if (Hc(m, n) == 1) E.emplace_back(m, n);
        }
    }
    return {N, E};
}
...
std::vector<std::pair<int, int>> trace_cycle(const std::vector<std::pair<int, int>>& E) {
    ...
    for (size_t i = 0; i < E.size() - 1; ++i) {
        ...
        for (int r : col_to_rows[current_col]) {
            if (r != current_row) { next_row = r; break; }
        }
        ...
        for (int c : row_to_cols[current_row]) {
            if (c != current_col) { next_col = c; break; }
        }
...
MatrixXi buildLinearSystem(
    ...
    for (int i = 0; i < var_count; ++i) pos_to_idx[Hc_nonzero[i]] = i;
    ...
    for (int m_prime = 0; m_prime < Hd.rows(); ++m_prime) {
        ...
        if (cycle_path.size() != E.size()) continue;
    ...
    if (equations.empty()) return MatrixXi::Zero(0, var_count + 1);
...
std::vector<int> solveDeltaRow(
    ...
    for(int j = 0; j < num_cols; ++j) if(!is_pivot_var[j]) free_vars.push_back(j);
